...
//
// ephapaxiser CLI — Enforce single-use linear type semantics on resources in existing code via Ephapax
...
/// ephapaxiser — Enforce single-use linear type semantics on resources in existing code via Ephapax
#[derive(Parser)]
...
enum Commands {
    ...
    /// Validate a ephapaxiser.toml manifest.
    Validate { #[arg(short, long, default_value = "ephapaxiser.toml")] manifest: String },
    /// Generate Ephapax wrapper, Zig FFI bridge, and C headers.
    Generate {
        #[arg(short, long, default_value = "ephapaxiser.toml")] manifest: String,
        #[arg(short, long, default_value = "generated/ephapaxiser")] output: String,
    },
    /// Build the generated artifacts.
    Build { #[arg(short, long, default_value = "ephapaxiser.toml")] manifest: String, #[arg(long)] release: bool },
    /// Run the workload.
    Run {
        #[arg(short, long, default_value = "ephapaxiser.toml")] manifest: String,
    ...
    Info { #[arg(short, long, default_value = "ephapaxiser.toml")] manifest: String },
}

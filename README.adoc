...
https://github.com/hyperpolymath[Ephapax] linear type wrappers around
...
_impossible_ in wrapped code.

Ephapax (from Greek _ἐφάπαξ_ — "`once for all`") is hyperpolymath’s
linear type language where every value must be used exactly once.
Ephapaxiser brings those guarantees to existing Rust, C, and Zig
codebases without requiring the developer to learn Ephapax itself.
...
[arabic]
. *Analyses* your source code for resource acquisition and release
patterns (file handles, sockets, database connections, GPU buffers,
crypto keys)
. *Generates* Ephapax linear type wrappers that structurally enforce
single-use semantics on each resource
. *Proves* linearity via the Idris2 ABI layer — dependent types
guarantee that every resource is consumed exactly once
. *Bridges* proofs into executable code via the Zig FFI — zero runtime
overhead, proofs are erased at compile time
. *Emits* target-language wrapper code that the developer can drop in
place
...
* *Double-free impossible* — consumption proof means the resource cannot
be freed twice
...
* *Database connection pooling* — connections checked out exactly once,
always returned
...
The Idris2 ABI layer (`+src/interface/abi/+`) is the formal
specification:

* `+Types.idr+` — `+LinearResource+`, `+UsageCount+`,
`+ResourceLifecycle+`, `+ConsumeProof+` — types that express "`this
...
____
[!IMPORTANT] *When Idris2 proofs conflict with Ephapax linear types,
Idris2 always wins.*
...
* *File handle management* — ensure every opened file is closed exactly
once
